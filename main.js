...
async (req, res) => {
 const errors = validationResult(req).errors;
   ...
   const {quantity} = req.body //quantity는 사용자가 사려고 하는 코인수량 string type.
   //todo (1) quantity는 Number, 소수점 4자리까지 받는다.
  const numberedQuantity = Number(parseFloat(quantity).toFixed(4));
  ...
  if (balance < totalPrice) {
    ...
    res.send({price: totalPrice, quantity: numberedQuantity}).status(200)
...
//코인 전량 구매하기
app.post('/coins/:coin_name/buyAll', setAuth,
body('all', 'true 혹은 false값을 입력해주세요').isBoolean(),
async (req, res) => {
  const errors = validationResult(req).errors;
  if (Object.keys(errors).length !== 0) { //error존재하면 400 bad request
    let messages = errors.map(error => error.msg)  
    return res.status(400).json({ errors: messages });
  }
  ...
  if (all) { //all이 true라면
    ...
    if (balance > 0) {
      ...
      await Asset.findOneAndUpdate({name: req.params.coin_name, user: userId},{balance: allQuantity});
    } else { //전량 구매하려는 개별 코인 가격이 잔고보다 클 때
...
async (req, res) => {
 const errors = validationResult(req).errors;
   ...
   const numberedQuantity = Number(quantity);
   console.log((numberedQuantity % 1).toString().length)
    if((numberedQuantity % 1).toString().length > 6) {
      return res.json({error: 'Please enter a decimal place with 4 digits or less '})
    }
   ...
   if (numberedQuantity > coinAsset.balance) {
     ...
     let newQuantity = parseFloat(coinAsset.balance - numberedQuantity).toFixed(4);
...
//코인 전량 판매하기
app.post('/coins/:coin_name/sellAll', setAuth,
body('all', 'true 혹은 false값을 입력해주세요').isBoolean(),
async (req, res) => {
  const errors = validationResult(req).errors;
  if (Object.keys(errors).length !== 0) { //error존재하면 400 bad request
  ...
  const {all} = req.body //request body의 all 
  if (all) { //all이 true라면 전량 판매가능
    const userId = req.userId; //로그인 된 유저id 가져온다 
    const [coinAsset] = await Asset.find({name: req.params.coin_name, user: userId});
    console.log(`coinAsset.balance ${coinAsset.balance}`)
    if (coinAsset.balance > 0) { //팔려고 하는 코인 잔고가 있을 때
      ...
      res.send({price: income, quantity: coinAsset.balance})
    } else {
      res.send({error: "you don't have any coin to sell."}).status(400);
    }
